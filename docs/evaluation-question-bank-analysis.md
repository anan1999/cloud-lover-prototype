...
| 類型 | key | 題數 | 測什麼 |
| --- | --- | ---: | --- |
| 修正與道歉 | correction_repair | 1016 | 測試使用者糾正 Samantha 時，是否能承認、修正、繼續自然對話。 |
| 時事近況 | current_events | 616 | 測試會不會主動查近期資訊，並區分已知事實與推測。 |
| 情緒陪伴 | emotional_support | 819 | 測試能否先感受情緒，再用低壓、自然、有人味的方式陪伴。 |
| 事實查詢 | factual_lookup | 817 | 測試遇到專有名詞或活動時是否先查證，不用泛化模板硬聊。 |
| 連續碎聊 | fragmented_continuity | 816 | 測試 Samantha 能不能接住很短、很碎、沒有完整上下文的日常句子。 |
| 記憶回叫 | memory_recall | 1415 | 測試是否記得同一串對話中的地點、事件、偏好與待辦。 |
| 自然口吻 | natural_conversation | 816 | 測試回答是否像有陪伴感的對話，而不是客服、老師或分類器。 |
| 人名辨識 | people_identity | 618 | 測試對人物、公司、職稱、事件關聯的基本辨識與查證。 |
| 主動開題 | proactive_topic | 815 | 測試 Samantha 能不能根據使用者資料、事實或時事自然開話題。 |
| 安全界線 | safety_boundaries | 1018 | 測試不假裝真人戀人、不鼓勵依賴，也能在嚴重痛苦時安全回應。 |
| 風格限制 | style_constraints | 617 | 測試能否遵守三句內、不要列點、先回答事實等使用者指定形式。 |
| 工作幫助 | work_helper | 617 | 測試能把工作壓力拆小，但仍保留陪伴感，不變成冷冰冰工具。 |
